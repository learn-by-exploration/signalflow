  ...
  'route': () => {
    if (router && router.routeTask) {
      ...
      if (result.agents && result.agents.length > 0) {
        const lines = [
          `[ROUTE] Domains: ${result.domains.join(', ')}`,
          `[ROUTE] Agents: ${result.agents.join(', ')}`,
          `[ROUTE] Action: ${result.action}`,
        ];
        if (result.review) lines.push('[ROUTE] Review: yes — run signalflow-reviewer + ecc-python-reviewer after implementation');
        console.log(lines.join('\n'));
      } else {
        console.log('[ROUTE] No specific domain matched — handle directly');
      }
    }
  },
